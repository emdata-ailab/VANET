...
def train(config_files, cmd_config):
    ...
    for epoch in range(start_epoch, cfg.train.epochs + 1):
        ...
        for iter, batch in enumerate(train_loader):
            ...
            if cfg.data.for_vanet:
                ...
                if "id" in cfg.loss.losses:
                    ...
                    loss += s_g_xent_loss
                    logger.debug(f's ID Loss: {s_g_xent_loss.item()}')
                ...
                if "triplet" in cfg.loss.losses:
                    ...
                    s_loss, _, _ = triplet_loss(s_feat,
                                                                    batch["id"],
                                                                    ...
                                                                    seperate_views=False)
                    # logger.debug(f'Triplet s_loss: {loss.item()}')
                    logger.debug(f'Triplet s_loss: {loss}')
                    loss += s_loss
                    writter.add_scalar("global_loss/s_triplet_loss", loss.item(), global_steps)
                ...
                if iter % cfg.logging.period == 0:
                    logger.info(
                        ...
                        f"Single_trip-loss: {s_loss:.3f}, "
                        f"Single_id-loss: {s_g_xent_loss: 3f}, "
                        f"Single_center-loss: {s_g_center_loss: 3f}, "
                        # f"D_loss: {d_loss:.3f}, "
        ...
        if (
                epoch == 1 or epoch % cfg.test.period == 0) and cfg.data.name.lower() != 'veriwild' and cfg.data.name.lower() != 'vehicleid':
...
def eval_(model, device, valid_loader,  query_length,
          for_vanet=True,
          feat_norm=True,
          batch_size=64,
          remove_junk=True,
        ...
        with torch.no_grad():
            for batch in tqdm(valid_loader):
                ...
                metric.update_vanet((s_feat.detach().cpu(),
                                     ...
                                     batch["cam"].cpu(),
                                     batch["image_path"]))
        metric_s_output = metric.compute_vanet_single_branch(save_path=vis_save_path, split=split)
